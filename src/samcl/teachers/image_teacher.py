...
class ImageTeacherConfig:
    # arch: "resnet50" (torchvision) or "clip" (transformers)
    arch: str = "resnet50"
    # only for arch="clip"
...
class FrozenImageTeacher(nn.Module):
    """
    Frozen vision encoder used ONLY for sampling-time similarity.
    ...
    def __init__(self, cfg: ImageTeacherConfig, device: torch.device) -> None:
        ...
        if self.mode == "resnet50":
            # NOTE: torchvision pretrained weights may require SSL access to download.pytorch.org.
            ...
            # Use CLIP vision encoder from HuggingFace (often available even when download.pytorch.org is blocked).
            try:
    ...
    def encode_images(self, pil_images: list) -> torch.Tensor:
        ...
        clip = self.backbone  # CLIPModel
        inputs = self.clip_processor(images=pil_images, return_tensors="pt")
        pixel_values = inputs["pixel_values"].to(self.device)

        # Compatible extraction similar to main model wrapper:
