...
def cosine_lr(optimizer, base_lr, warmup_length, steps):
    def _lr_adjuster(step):
        if step < warmup_length:
            ...
            # Clamp so e/es stays in [0, 1]: after the scheduled horizon, LR stays at 0.
            # Without this, e/es > 1 makes cos(pi*e/es) oscillate (bad if global_step exceeds total_steps).
            if es <= 0:
                lr = 0.0
            else:
                e = min(e, es)
