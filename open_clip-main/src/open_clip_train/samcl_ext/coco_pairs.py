...
class CocoPairsDataset(Dataset):
    """
    COCO (image, caption) positives — same contract as ``src/samcl/data/coco_pairs.py``:
    each row is one annotation; ``image_id`` / ``caption_id`` match COCO ids for semantic relations.
    """
